...
init -999 python in _viewers:
    ...
    xygroup = {"position": ("xpos", "ypos"), "anchor": ("xanchor", "yanchor"), "offset": ("xoffset", "yoffset")}
...
init 999 python in _viewers:
    ...
    def scene_editor_safe_transform_value(prop, value):
        if value is None:
            default = property_default_value.get(prop, None)
            if default is not None:
                return default
        return value

    def scene_editor_ensure_around_defaults(tran):
        if getattr(tran, "xaround", None) is None:
            setattr(tran, "xaround", property_default_value.get("xaround", 0))
        if getattr(tran, "yaround", None) is None:
            setattr(tran, "yaround", property_default_value.get("yaround", 0))
...
init -997 python in _viewers:
    ...
    def viewer_transform(tran, st, at, camera_check_points, image_check_points, scene_checkpoints, viewer_check_points,
        ...
        preview_zoom = preview_size
        preview_offset = (1 - preview_size) / 2
        preview_offset_y = preview_offset
        if "scene_editor_preview_scale" in globals():
            preview_zoom = preview_size * scene_editor_preview_scale
            preview_offset = max(0, (1 - preview_zoom) / 2)
            preview_offset_y = preview_offset
        leftover = 1 - preview_zoom
        if "scene_editor_preview_offset_x" in globals():
            preview_offset = leftover * min(1.0, max(0.0, scene_editor_preview_offset_x))
        else:
            preview_offset = leftover / 2
        if "scene_editor_preview_offset_y" in globals():
            preview_offset_y = leftover * min(1.0, max(0.0, scene_editor_preview_offset_y))
        else:
            preview_offset_y = leftover / 2
        if not persistent._viewer_legacy_gui:
            if aspect_16_9:
                box.add(Transform(zoom=preview_zoom, xpos=preview_offset, ypos=preview_offset_y)(child))
                if persistent._viewer_rot:
                    for i in range(1, 3):
                        box.add(Solid("#00ff2a", xsize=preview_zoom, ysize=1, xpos=preview_offset, ypos=preview_zoom*i/3+preview_offset_y))
                        box.add(Solid("#00ff2a", xsize=1, ysize=preview_zoom, xpos=preview_zoom*i/3+preview_offset, ypos=preview_offset_y))
            else:
                box.add(Transform(zoom=preview_zoom, xpos=preview_offset, ypos=preview_offset_y)(child))
                if persistent._viewer_rot:
                    for i in range(1, 3):
                        box.add(Solid("#F00", xsize=preview_zoom, ysize=1, ypos=preview_zoom*i/3+preview_offset_y))
                        box.add(Solid("#F00", xsize=1, ysize=preview_zoom, xpos=preview_zoom*i/3+preview_offset))
        else:
            box.add(child)
            if persistent._viewer_rot:
                for i in range(1, 3):
                ...
                if looped_time >= scene_start and looped_time < checkpoint:
                    ...
                    if p not in ("child", "function", "at_list", "props_use_default"):
                        ...
                        if p in props_groups["focusing"]:
                            ...
                            if p in ("radius", "angle", "xaround", "yaround"):
                                scene_editor_ensure_around_defaults(tran)
                            setattr(tran, p, scene_editor_safe_transform_value(p, v))
                    break
            else:
    ...
    _scene_editor_image_name_candidates_cache = None

    def scene_editor_clear_image_name_cache():
        global _scene_editor_image_name_candidates_cache
        _scene_editor_image_name_candidates_cache = None

    def get_image_name_candidates():
        global _scene_editor_image_name_candidates_cache
        if _scene_editor_image_name_candidates_cache is not None:
            return list(_scene_editor_image_name_candidates_cache)
        ...
        for n, d in images.items():
            if isinstance(d, renpy.store.Live2D):
                ...
                for i in range(1,len(nonexclusive)+1):
                    for comb in combinations(nonexclusive, i):
                        nonexclusive_sets.append(comb)
                filtered_nonexclusive_sets = set(nonexclusive_sets)
                if attribute_filter is not None:
                    filtered_nonexclusive_sets = set()
                    for nes in nonexclusive_sets:
                        filtered = attribute_filter(nes)
                        if filtered is None:
                            continue
                        filtered_nonexclusive_sets.add(tuple(filtered))
                ...
                result.append(n)
        _scene_editor_image_name_candidates_cache = tuple(result)
        return result
